...
$(function(){
	...
	$('form[name=ADUpdateForm]').submit(function(){
		...
		$(this).attr('action','${pageContext.request.contextPath}/adReference/updateADReferenceinfo.do');
	...
	$('#deleteBtn').click(function(){
		$(location).attr('href','${pageContext.request.contextPath}/adReference/deleteADReferenceinfo.do?ref_no=${referenceInfo.ref_no}');
	});
});
		...
		<div id="myCarousel" class="carousel slide" data-ride="carousel">
			...
			<c:forEach items="${referenceInfo.attachFileRoomList}" var="fileItem" varStatus="stat">
				...
				<img src="http://localhost/file/${fileItem.file_save_name}" alt="pic${stat.count}"
					onclick="javascript:location.href='${pageContext.request.contextPath}/adReference/fileDownload.do?file_seq=${fileItem.file_seq}'">
				</div>
			</c:forEach>
			</div>
