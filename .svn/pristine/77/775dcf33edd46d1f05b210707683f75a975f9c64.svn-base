...
$(function(){
    ...
    $('#qa_content').summernote('code','${qaInfo.qa_content}');
    
    $('form').submit(function(){
    	var contentVal = $('#qa_content').summernote('code');
    	$(this).append('<input type="textarea" name="qa_content" value="'+contentVal+'"/>');
    	$(this).append('<input type="hidden" name="qa_no" value="${qaInfo.qa_no}"/>');
    });
    
    $('#reply').click(function(){
    	var $frm = $('<form action="${pageContext.request.contextPath }/user/qaboard/qaboardReplyForm.do" method="post"/>');
    	$frm.append('<input type="hidden" name="qa_no" value="${qaInfo.qa_no}"/>');
    	$frm.append('<input type="hidden" name="rnum" value="${qaInfo.rnum}"/>');
    ...
    $('#delete').click(function(){
    	$(location).attr('href','${pageContext.request.contextPath}/user/qaboard/deleteQABoardInfo.do?qa_no=${qaInfo.qa_no}');
    });
});
</script>
</head>
<body>
<form id="test" class="form-horizontal" role="form" action="${pageContext.request.contextPath }/user/qaboard/updateQABoardInfo.do" method="post">
	<div class="form-group">
		<input type="hidden" name="rnum" value="${qaInfo.rnum }">
		<label class="control-label col-sm-2" for="qa_title">제목:</label>
		<div class="col-sm-10">
			<input type="text" class="form-control" id="qa_title" name="qa_title" value="${qaInfo.qa_title }">
		</div>
	</div>
	<div class="form-group">
		<label class="control-label col-sm-2" for="qa_nickname">작성자 대화명:</label>
		<div class="col-sm-10"> 
			<input type="text" class="form-control" id="qa_nickname" name="qa_nickname" value="${qaInfo.qa_nickname }">
		</div>
	</div>
	<div class="form-group">
	...
	<div class="form-group"> 
		<div class="col-sm-offset-2 col-sm-10">
			...
			<c:if test="${LOGIN_MEMBERINFO.mem_name == qaInfo.qa_nickname }">
